        ...
        builder: (context, snapshot) {
          ...
          return ListView(
            ...
            children: [
              ...
              RangeUsageAnalysisCard(deviceRef: deviceRef),
            ],
          );
        },
      ),
...
class RangeAnalysisResult {
  final List<UsageLogPoint> points;
  ...
  final double averageEnergyPerHour;
  final UsageLogPoint? maxPowerPoint;
  final UsageLogPoint? minPowerPoint;
  final UsageLogPoint? maxCurrentPoint;
  ...
  RangeAnalysisResult({
    ...
    required this.averageEnergyPerHour,
    required this.maxPowerPoint,
    required this.minPowerPoint,
    required this.maxCurrentPoint,
  });
}

class RangeUsageAnalysisCard extends StatefulWidget {
  final DatabaseReference deviceRef;

  const RangeUsageAnalysisCard({
    super.key,
    required this.deviceRef,
  });

  @override
  State<RangeUsageAnalysisCard> createState() => _RangeUsageAnalysisCardState();
}

class _RangeUsageAnalysisCardState extends State<RangeUsageAnalysisCard> {
  DateTime selectedDate = DateTime.now();
  TimeOfDay startTime = const TimeOfDay(hour: 0, minute: 0);
  TimeOfDay endTime = const TimeOfDay(hour: 23, minute: 59);

  bool loading = false;
  String message = 'Select date/time and press Analyze.';
  ...
  DateTime makeDateTime(DateTime date, TimeOfDay time) {
    return DateTime(date.year, date.month, date.day, time.hour, time.minute);
  }

  String shortTimeFromEpoch(int epoch) {
  ...
  UsageLogPoint pointFromMap(Map<String, dynamic> item) {
    return UsageLogPoint(
      epoch: asInt(item['epoch']),
      dateTime: asString(item['dateTime'], asString(item['time'], 'unknown')),
      voltage: asDouble(item['voltage']),
      current: asDouble(item['current']),
      power: asDouble(item['power']),
      energyKWh: asDouble(item['energyKWh']),
      cost: asDouble(item['cost']),
    );
  }

  RangeAnalysisResult buildResult(
    List<UsageLogPoint> allPoints,
    int startEpoch,
    int endEpoch,
  ) {
    allPoints.sort((a, b) => a.epoch.compareTo(b.epoch));

    final selected = allPoints
        .where((p) => p.epoch >= startEpoch && p.epoch <= endEpoch)
        .toList();

    UsageLogPoint? previousPoint;

    for (final point in allPoints) {
      if (point.epoch < startEpoch) {
        previousPoint = point;
      } else {
        break;
      }
    ...
    UsageLogPoint? lastPoint = previousPoint;

    for (final point in selected) {
      if (lastPoint != null) {
        final energyDelta = point.energyKWh - lastPoint.energyKWh;
        final costDelta = point.cost - lastPoint.cost;

        if (energyDelta > 0) energyUsed += energyDelta;
        if (costDelta > 0) costUsed += costDelta;
      }

      lastPoint = point;
    }

    if (selected.length == 1 && previousPoint == null) {
      energyUsed = selected.first.energyKWh;
      costUsed = selected.first.cost;
    }

    final durationHours =
        (endEpoch - startEpoch).clamp(1, 999999999) / 3600.0;

    final averageCurrent = selected.isEmpty
        ? 0.0
        : selected.map((p) => p.current).reduce((a, b) => a + b) /
            selected.length;

    final averagePower =
        energyUsed > 0 ? (energyUsed / durationHours) * 1000.0 : 0.0;

    final averageCostPerHour = costUsed / durationHours;
    final averageEnergyPerHour = energyUsed / durationHours;

    UsageLogPoint? maxPowerPoint;
    UsageLogPoint? minPowerPoint;
    UsageLogPoint? maxCurrentPoint;

    for (final point in selected) {
      if (maxPowerPoint == null || point.power > maxPowerPoint.power) {
        maxPowerPoint = point;
      }

      if (minPowerPoint == null || point.power < minPowerPoint.power) {
        minPowerPoint = point;
      }

      if (maxCurrentPoint == null || point.current > maxCurrentPoint.current) {
    ...
    return RangeAnalysisResult(
      ...
      averageEnergyPerHour: averageEnergyPerHour,
      maxPowerPoint: maxPowerPoint,
      minPowerPoint: minPowerPoint,
      maxCurrentPoint: maxCurrentPoint,
    );
  }

  Future<void> analyzeRange() async {
    final start = makeDateTime(selectedDate, startTime);
    final end = makeDateTime(selectedDate, endTime);
    ...
    setState(() {
      loading = true;
      message = 'Reading usage logs...';
      result = null;
    });

    try {
      final snap =
          await widget.deviceRef.child('usageLogs/${dateKey(selectedDate)}').get();

      final map = snapMap(snap);
      final points = <UsageLogPoint>[];

      for (final value in map.values) {
        final item = dynamicMap(value);
        if (item.isNotEmpty) {
          final point = pointFromMap(item);
      ...
      if (points.isEmpty) {
        setState(() {
          result = null;
          message =
              'No usage log data for ${formatDate(selectedDate)}. Upload the new Arduino code and keep device online for at least 1 minute.';
        });
        return;
      }
      ...
      final analysis = buildResult(points, startEpoch, endEpoch);

      setState(() {
        result = analysis;
        message = analysis.points.isEmpty
            ? 'No data points inside selected time range.'
            : 'Analysis completed. Points: ${analysis.points.length}';
      });
    } catch (e) {
      setState(() {
        result = null;
  ...
  Widget smallResultCard(String title, String value, IconData icon, Color color) {
    return GlassCard(
  ...
  Widget pointText(String title, UsageLogPoint? point, String unit) {
    if (point == null) {
      return Text('$title: no data');
    }

    final value = unit == 'A' ? point.current : point.power;

    return Text(
      '$title: ${value.toStringAsFixed(unit == 'A' ? 2 : 1)} $unit at ${shortTimeFromEpoch(point.epoch)}',
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    final chartWidth = currentResult == null
        ? 720.0
        : math.max(720.0, currentResult.points.length * 46.0);
    ...
    return GlassCard(
      child: Column(
        ...
        children: [
          const Text(
            'Date / Time Usage Analysis',
            style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold),
          ),
          const SizedBox(height: 6),
          Text(
            'Choose a date and time duration to calculate energy, cost, average power, maximum power time, and minimum power time.',
            style: TextStyle(color: Colors.white.withOpacity(0.65)),
          ),
          const SizedBox(height: 14),
          Wrap(
            ...
            children: [
              ...
              FilledButton.icon(
                onPressed: loading ? null : pickStartTime,
                icon: const Icon(Icons.play_arrow),
                label: Text('Start ${formatTimeOfDay(startTime)}'),
              ),
              FilledButton.icon(
                onPressed: loading ? null : pickEndTime,
                icon: const Icon(Icons.stop),
                label: Text('End ${formatTimeOfDay(endTime)}'),
              ),
            ],
          ),
          const SizedBox(height: 12),
          SizedBox(
            ...
            child: FilledButton.icon(
              onPressed: loading ? null : analyzeRange,
              icon: loading
                  ? const SizedBox(
                      width: 18,
                      height: 18,
              ...
              label: Text(loading ? 'Analyzing...' : 'Analyze Selected Time'),
            ),
          ),
          const SizedBox(height: 12),
          Text(
          ...
          if (currentResult != null && currentResult.points.isNotEmpty) ...[
            const SizedBox(height: 16),
            GridView.count(
              crossAxisCount: 2,
              shrinkWrap: true,
              ...
              children: [
                smallResultCard(
                  'Energy Used',
                  '${currentResult.energyUsed.toStringAsFixed(5)} kWh',
                  Icons.battery_charging_full,
                  Colors.cyanAccent,
                ),
                smallResultCard(
                  'Cost Used',
                  'Rs ${currentResult.costUsed.toStringAsFixed(2)}',
                  Icons.payments,
                  Colors.greenAccent,
                ),
                smallResultCard(
                  'Avg Power',
                  '${currentResult.averagePower.toStringAsFixed(1)} W',
                  Icons.bolt,
                  Colors.blueAccent,
                ),
                smallResultCard(
                  'Avg Current',
                  '${currentResult.averageCurrent.toStringAsFixed(2)} A',
                  Icons.waves,
                  Colors.orangeAccent,
                ),
                smallResultCard(
                  'Avg Energy/h',
                  '${currentResult.averageEnergyPerHour.toStringAsFixed(5)} kWh/h',
                  Icons.speed,
                  Colors.purpleAccent,
                ),
                smallResultCard(
                  'Avg Cost/h',
                  'Rs ${currentResult.averageCostPerHour.toStringAsFixed(2)}',
                  Icons.trending_up,
                  Colors.tealAccent,
                ),
              ],
            ...
            GlassCard(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  pointText('Maximum power', currentResult.maxPowerPoint, 'W'),
                  const SizedBox(height: 5),
                  pointText('Minimum power', currentResult.minPowerPoint, 'W'),
                  const SizedBox(height: 5),
                  pointText('Maximum current', currentResult.maxCurrentPoint, 'A'),
                ],
              ),
            ),
            const SizedBox(height: 4),
            Text(
              'Power graph for selected duration. Red = maximum power, orange = minimum power. Swipe left/right.',
              style: TextStyle(color: Colors.white.withOpacity(0.65)),
            ),
            const SizedBox(height: 10),
            SingleChildScrollView(
              ...
              child: SizedBox(
                ...
                width: chartWidth,
                child: CustomPaint(
                  painter: RangePowerChartPainter(currentResult.points),
                ),
              ),
            ),
          ],
...
class RangePowerChartPainter extends CustomPainter {
  final List<UsageLogPoint> points;

  RangePowerChartPainter(this.points);
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final maxPower =
        points.map((p) => p.power).fold<double>(0.0, math.max);
    final safeMax = maxPower <= 0 ? 1.0 : maxPower;

    UsageLogPoint maxPoint = points.first;
    UsageLogPoint minPoint = points.first;

    for (final point in points) {
      if (point.power > maxPoint.power) maxPoint = point;
      if (point.power < minPoint.power) minPoint = point;
    }
    ...
    for (int i = 1; i <= 4; i++) {
      ...
      textPainter.text = TextSpan(
        text: '${labelValue.toStringAsFixed(0)}W',
        style: TextStyle(
    ...
    for (int i = 0; i < points.length; i++) {
      ...
      final y =
          chartRect.bottom - (points[i].power / safeMax).clamp(0.0, 1.0) * chartRect.height;
    ...
    for (int i = 0; i < points.length; i++) {
      ...
      if (point.epoch == maxPoint.epoch) {
        canvas.drawCircle(offset, 6, maxPaint);
        drawLabel(
          canvas,
          textPainter,
          chartRect,
          offset,
          'Max ${point.power.toStringAsFixed(0)}W',
        );
      } else if (point.epoch == minPoint.epoch) {
        canvas.drawCircle(offset, 5, minPaint);
        drawLabel(
          canvas,
          textPainter,
          chartRect,
          offset,
          'Min ${point.power.toStringAsFixed(0)}W',
        );
      } else {
        canvas.drawCircle(offset, 3.2, pointPaint);
      }

      if (i == 0 || i == points.length - 1 || i % 10 == 0) {
        final dt = DateTime.fromMillisecondsSinceEpoch(point.epoch * 1000);
        ...
        textPainter.paint(
          canvas,
          Offset(offset.dx - textPainter.width / 2, chartRect.bottom + 14),
        );
      }
    }
  }
  ...
  void drawLabel(
    ...
    final x = (point.dx - textPainter.width / 2)
        .clamp(chartRect.left, chartRect.right - textPainter.width);
    final y = (point.dy - 24)
        .clamp(chartRect.top, chartRect.bottom - textPainter.height);
  ...
  @override
  bool shouldRepaint(covariant RangePowerChartPainter oldDelegate) {
    return oldDelegate.points != points;
  }
}
